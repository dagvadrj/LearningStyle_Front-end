...
const DEFAULT_LESSON_IMAGE = { uri: "https://via.placeholder.com/80x80?text=Lesson" };
...
const getWeatherInfo = (weatherData) => {
  // ... (Энэ функц хэвээрээ байна) ...
    ...
    switch (description) {
        case "clear sky": cloudText = "Цэлмэг"; iconName = "weather-sunny"; break;
        case "few clouds":
        case "scattered clouds":
        case "broken clouds":
        case "overcast clouds": cloudText = "Үүлэрхэг"; iconName = "weather-partly-cloudy"; break;
        case "rain":
        case "light rain":
        case "moderate rain": cloudText = "Бороотой"; iconName = "weather-rainy"; break;
        case "drizzle":
        case "light drizzle": cloudText = "Шиврээ бороо"; iconName = "weather-partly-rainy"; break;
        case "thunderstorm": cloudText = "Аадар бороо"; iconName = "weather-lightning-rainy"; break;
        case "snow":
        case "light snow":
        case "heavy snow": cloudText = "Цастай"; iconName = "weather-snowy"; break;
        case "mist":
        case "fog":
        case "haze": cloudText = "Манантай"; iconName = "weather-fog"; break;
        case "squall": cloudText = "Салхитай"; iconName = "weather-windy"; break;
        default: cloudText = "Тодорхойгүй"; iconName = "weather-cloudy-alert"; break;
    }
...
const getLessonIconInfo = (lessonName) => {
  ...
  switch (lessonName.toLowerCase()) {
    case "математик":
      return { name: "calculator-outline", library: "Ionicons", color: LESSON_COLORS.math };
    case "англи хэл":
      return { name: "language-outline", library: "Ionicons", color: LESSON_COLORS.english };
    case "мэдээллийн технологи":
      return { name: "laptop-outline", library: "Ionicons", color: LESSON_COLORS.it };
    case "уран зохиол":
      return { name: "book-outline", library: "Ionicons", color: LESSON_COLORS.literature };
    case "монгол бичиг":
      // 'script-text-outline' (MCI) эсвэл 'pencil-outline' (Ionicons)
      return { name: "pencil-outline", library: "Ionicons", color: LESSON_COLORS.mongol_bichig };
    case "монгол хэл":
      return { name: "chatbubble-ellipses-outline", library: "Ionicons", color: LESSON_COLORS.mongol_hel };
    case "дизайн зураг зүй, технологи":
      return { name: "color-palette-outline", library: "Ionicons", color: LESSON_COLORS.design };
    case "орос хэл":
      // Англи хэлтэй ижил icon, өөр өнгөөр
      return { name: "language-outline", library: "Ionicons", color: LESSON_COLORS.russian };
    case "эрүүл мэнд":
      return { name: "medkit-outline", library: "Ionicons", color: LESSON_COLORS.health };
    case "иргэний ёс зүйн боловсрол":
      return { name: "school-outline", library: "Ionicons", color: LESSON_COLORS.civics };
...
const renderIcon = (iconInfo, size = 30, defaultColor = '#000') => {
  ...
  return <Ionicons name={DEFAULT_LESSON_ICON.name} size={size} color={DEFAULT_LESSON_ICON.color} />; // Default fallback
};
...
const LessonGridItem = React.memo(({ item, navigation, onShowMorePress }) => {
 ...
 if (item.isMore) {
    return (
      ...
      >
        ...
        <Text style={styles.gridItemText} numberOfLines={1}>Бүгд</Text>
      </TouchableOpacity>
    );
  }
 ...
 return (
    <TouchableOpacity
      style={styles.gridItemTouchable}
      onPress={() =>
        navigation.navigate("LessonLearningType", { lessonId: item._id, lessonName: item.lessonName })
      }
      activeOpacity={0.7}
    >
      <View style={[styles.gridItemIconContainer, { backgroundColor: iconInfo.color }]}>
        {renderIcon({...iconInfo, iconColor: 'white'}, 30)}
      </View>
      <Text style={styles.gridItemText} numberOfLines={1}>{item.lessonName || "Хичээл"}</Text>
    </TouchableOpacity>
  );
});
...
const ClassLessonItem = React.memo(({ item, navigation }) => {
  ...
  return (
    ...
    onPress={() =>
      navigation.navigate("LessonDetail", { lessonId: item._id, lessonName: item.lessonName })
    }
    activeOpacity={0.7}
  >
    ...
    <Ionicons name="chevron-forward-outline" size={20} color="#ccc" style={styles.classLessonArrow}/>
  </TouchableOpacity>
  );
});
...
const HomeScreen = ({ navigation }) => {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
        ...
        try {
          ...
          const [userResponse, lessonsResponse, weatherResponse] = await Promise.all([
          ...
          if (error.message === "AUTH_TOKEN_MISSING" || error.response?.status === 401 || error.response?.status === 403) {
              Alert.alert("Нэвтрэлт амжилтгүй", "Хандах эрхгүй эсвэл токен хүчингүй болсон. Дахин нэвтэрнэ үү.", [
  ...
  }, [navigation]); // navigation added for logout
  ...
  const gridDisplayData = useMemo(() => {
    ...
    if (lessonsToShow.length > 7) { // 7 хичээл + "Бүгд" товч
  ...
  // --- Render Logic ---
  if (loading && !refreshing) { // Зөвхөн анхны load үед бүтэн дэлгэцээр харуулах
    return (
  ...
  if (error && !user && !refreshing) { // Хэрэглэгчгүй, refresh хийгээгүй үед алдаа харуулах
    return (
      <SafeAreaView style={styles.centeredContainer}>
        ...
        >
            ...
            <View style={styles.headerTopRow}>
                <View style={styles.weatherContainer}>
                    <MaterialCommunityIcons name={weatherInfo.icon} size={22} color="white" style={styles.weatherIcon} />
                    <Text style={styles.weatherText} numberOfLines={1}>
                        {weatherInfo.text}
                    </Text>
                </View>
                <TouchableOpacity onPress={() => Alert.alert("Мэдэгдэл", "Шинэ мэдэгдэл алга.")} style={styles.notificationButton}>
            ...
            >
                ...
                <Ionicons name="chevron-forward-outline" size={24} color="white" style={styles.walletArrow} />
            </TouchableOpacity>
        </LinearGradient>
        ...
        >
            ...
            <View style={styles.sectionContainer}>
                <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                >
                    <Text style={styles.testButtonText}>Тест Өгч Арга Барилаа Тодорхойлох</Text>
                </TouchableOpacity>
  ...
  <FlatList
    ...
    ListEmptyComponent={
      !loading && <Text style={styles.emptyListText}>Хичээл олдсонгүй.</Text>
    }
    ...
    renderItem={({ item }) => {
        ...
        if (!item) {
            console.warn("Grid FlatList renderItem received invalid item:", item);
            return null;
        }
      ...
      <Text style={styles.collapseButtonText}>Хураах</Text>
            ...
            <View style={[styles.sectionContainer, { marginBottom: 20 }]}>
                ...
                <Text style={styles.sectionTitle}>
                     {user?.class ? `${user.class}-р Ангийн Хичээлүүд` : "Санал Болгох Хичээлүүд"}
                </Text>
                <FlatList
                    data={classLessons}
                    ListEmptyComponent={!loading && (
                        <View style={styles.emptyListContainer}>
                            <Ionicons name="library-outline" size={35} color="#ccc" style={styles.emptyListIcon}/>
                            <Text style={styles.emptyListText}>
                                {user?.class ? `${user.class}-р ангийн хичээл одоогоор алга байна.` : "Танд санал болгох хичээл алга байна."}
                            </Text>
                        </View>
                    )}
                    renderItem={({item}) => <ClassLessonItem item={item} navigation={navigation}/>}
...
const styles = StyleSheet.create({
  ...
  gridItemIconContainer: {
    ...
    shadowOpacity: 0.20, // Бага зэрэг сүүдэр нэмсэн
    shadowRadius: 2.62,
